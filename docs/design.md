...
**Stage 2 (rollout adaptation) — briefly promoted at M4, then UN-PROMOTED. Stays deferred.**

The measurement that promoted it: conditioning on a *generated* prefix is worse than conditioning on *nothing* (45.9% vs 53.9% at `eta = 1`), and the penalty widens as the sampler improves (5.0pp → 8.0pp). That looks like exposure bias, and rollout adaptation is the standard fix for exposure bias.

**It is not exposure bias.** Rollout adaptation makes a model robust to its OWN errors in context. The same-model arm says there are no such errors to be robust to: handing the model the *correct* prefix instead of its own buys ~3pp in the mid magnitude bins and nothing in the small ones, with near-identical magnitude histograms. `correct ≈ generated` means the prefix is uninformative, and no amount of training on the model's own rollouts makes an uninformative prefix informative.

**Reconciling this with the loss gap, which said the opposite.** Destroying the prefix costs +1.42 nats at b6 — the prefix strongly predicts the next LATENT. And it barely predicts arithmetic CORRECTNESS. Both measurements are true and they answer different questions: the prefix constrains magnitude range and local style, which is most of the latent's variance, and it does not constrain whether the sum is right. A conditioning signal can be highly informative about the representation and nearly uninformative about the property being scored. That is the general lesson, and it is why the loss gap was never evidence about truth.

The likeliest reading is that the prefix is uninformative *because the task is underdetermined*: with no question, "the next valid arithmetic step" has no right answer, so nothing ties block b+1 to block b beyond plausibility. Question conditioning at M5 supplies exactly that constraint. Revisit stage 2 after M5, on a model whose prefix has something to be right about.

...
| **RCD** — arXiv 2601.22954 | Converts discarded token representations into contextual residuals, injected into the next denoising step. | **Verified.** Prior art for the FIX, not the measurement. Addresses information lost when a token is committed; says nothing about whether downstream state supports reconstruction of an earlier step. |
| **Relay** — arXiv 2605.22967 | Carries last-layer hidden states forward through a differentiable per-token channel, trained with truncated BPTT. Frames the problem as the "hard reset" between denoising rounds. | **Verified.** The same problem this work measured from the other side — our refinement loop SHREDS context (`preserved` 0.10% -> 0.00% over three passes) because each pass restarts from committed discrete output. Relay's carry-forward IS the content/carrier separation proposed here for the relay experiment, already implemented and trained. Prior art for that experiment; not prior art for the provenance control. |
